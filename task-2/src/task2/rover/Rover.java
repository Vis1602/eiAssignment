    ...
    public void move() {
        ...
        if (!grid.isValidPosition(newX, newY)) {
            if (newX < 0 || newX >= grid.getWidth() || newY < 0 || newY >= grid.getHeight()) {
                System.out.println("Boundary reached. Cannot move outside the grid.");
            } else if (grid.hasObstacle(newX, newY)) {
                System.out.println("Obstacle found at (" + newX + ", " + newY + "). Cannot move.");
            }
            return;
        }
